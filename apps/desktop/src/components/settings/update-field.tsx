...
export function UpdateField(): ReactElement {
  ...
  const action: {
    ...
    switch (state.phase) {
      case 'checking':
        return { label: 'Verificando…', icon: RefreshCw, run: undefined, spinning: true }
      case 'available':
        return { label: `Instalar ${state.version}`, icon: ArrowDownToLine, run: install }
      case 'downloading':
        return {
          label: `Descargando${state.percent !== null ? ` ${state.percent}%` : '…'}`,
          icon: ArrowDownToLine,
          run: undefined,
        }
      case 'ready':
        return { label: 'Reiniciar para actualizar', icon: RotateCw, run: restart }
      case 'error':
        ...
        return state.during === 'install'
          ? { label: 'Reintentar instalación', icon: ArrowDownToLine, run: install }
          : { label: 'Buscar actualizaciones', icon: RefreshCw, run: checkNow }
      default:
        return { label: 'Buscar actualizaciones', icon: RefreshCw, run: checkNow }
    }
  })()
  ...
  return (
    <SettingsField
      ...
      description="Reflect busca versiones nuevas al iniciar y las instala solo cuando tú lo indicas."
    >
